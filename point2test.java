...
public class Point2Test {
	...
	@Test
	public void test11_hashCode() {
		double x = 5.0;
		double y = -9.0;
		Point2 p = new Point2(x, y);
		int expected = Objects.hash(x, y);
		
		assertEquals("p.hashCode() returned the wrong hash code; are you using Objects.hash?\n", 
				expected, p.hashCode());
	}
	
	@Test
	public void test12_equals() {
		Point2 p = new Point2();
		assertFalse("p.equals(null) did not return false!\n", 
				p.equals(null));
	}
	
	@Test
	public void test13_equals() {
		Point2 p = new Point2();
		assertTrue("p.equals(p) did not return true!\n", 
				p.equals(p));
	}
	
	@Test
	public void test14_equals() {
		Point2 p = new Point2();
		assertFalse("p.equals(\"hello\") did not return false!\n", 
				p.equals("hello"));
	}
	...
	public void test15_equals() {
		...
		double y = -33.3;
		Point2 p = new Point2(x, y);
		Point2 q = new Point2(x, y);
		assertTrue("p.equals(q) did not return true for equal p and q\n", 
				p.equals(q));
	}
	...
	public void test16_equals() {
		...
		assertFalse("p.equals(q) did not return false for unequal p and q\n", 
				p.equals(q));
		
		q = new Point2(x + Math.ulp(x), y);
		assertFalse("p.equals(q) did not return false for unequal p and q\n", 
				p.equals(q));
	}
}